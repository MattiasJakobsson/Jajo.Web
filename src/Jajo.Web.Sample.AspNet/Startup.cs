...
using Jajo.Web.Authorization;
using Jajo.Web.Configuration;
using Jajo.Web.Endpoints;
using Jajo.Web.ExceptionManagement;
using Jajo.Web.Http;
using Jajo.Web.ModelBinding;
using Jajo.Web.Output;
using Jajo.Web.Output.Spark;
using Jajo.Web.PartialRequests;
using Jajo.Web.RequestBranching;
using Jajo.Web.Routing.Superscribe;
using Jajo.Web.Routing.Superscribe.Conventional;
using Jajo.Web.Sample.AspNet;
using Jajo.Web.StructureMap;
using Jajo.Web.UnitOfWork;
using Jajo.Web.Validation;
using Owin;
using StructureMap;

[assembly: OwinStartup(typeof(Startup))]
namespace Jajo.Web.Sample.AspNet
{
        ...
        public void Configuration(IAppBuilder app)
        {
            ...
            container.Configure(x =>
            {
                x.Scan(y =>
                {
                    ...
                    y.AddAllTypesOf<IJajoUnitOfWork>();
            ...
            {
                {"jajo.StructureMap.Container", container}
            };
            ...
            app.Use<RedirectToCorrectUrl>(new RedirectToCorrectUrlOptions(y => y.ToLower()))
                .Use<BranchRequest>(new BranchRequestConfiguration()
                    .AddCase(y => y.GetException() != null, (AppFunc)app
                            ...
                            .Build(typeof(AppFunc)))
                    .AddCase(y => y.Get<bool>("jajo.AuthorizationFailed"), (AppFunc)app
                            .New()
                            .Use<SetStatusCode>(401)
                            .Use<HandleUnauthorizedMiddleware>()
                            .Build(typeof(AppFunc)))
                    .AddCase(y => !(y.Get<ValidationResult>("jajo.ValidationResult") ?? new ValidationResult(new List<ValidationResult.ValidationError>())).IsValid, (AppFunc)app
                            .New()
    ...
    public class TestMiddleware
    {
        ...
        public async Task Invoke(IDictionary<string, object> environment)
        {
            ...
            environment["jajo.Output"] = "Testing if statement";
    ...
    public class HandledExceptionMiddleware
    {
        ...
        public async Task Invoke(IDictionary<string, object> environment)
        {
            var exception = environment.Get<Exception>("jajo.Exception");

            await environment.WriteToOutput(exception.Message);

            environment["jajo.Output"] = exception.Message;
    ...
    public class HandleNotFoundMiddleware
    {
        ...
        public async Task Invoke(IDictionary<string, object> environment)
        {
            ...
            environment["jajo.Output"] = "Not found!";
    ...
    public class HandleValidationErrorMiddleware
    {
        ...
        public async Task Invoke(IDictionary<string, object> environment)
        {
            ...
            var validationResult = environment.Get<ValidationResult>("jajo.ValidationResult");
            ...
            environment["jajo.Output"] = result.ToString();
    ...
    public class HandleUnauthorizedMiddleware
    {
        ...
        public async Task Invoke(IDictionary<string, object> environment)
        {
            ...
            environment["jajo.Output"] = "Unauthorized";
